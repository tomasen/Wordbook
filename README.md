...
The daily review image uses deterministic, collision-free word packing. Words are placed largest-first so smaller labels can fill the remaining gaps, and the cluster is scored toward the canvas aspect ratio instead of the former strongly horizontal spiral. The layout then adjusts its global font scale to use the available square more fully while keeping every label inside a uniform inset. Equal review weights receive a medium size, while mixed weights use a square-root curve from 20 to 60 points so medium-priority words remain visible without obscuring the hardest words.

The cache key includes the words, measured label sizes, canvas size, and font scale. This keeps rotation and repeated share renders stable without reusing positions from an incompatible layout.
...
The normal interface is gated until the bundled models are initialized and both the local tutor and natural voice have completed a discarded warm-up inference. The preparation screen reports named stages such as checking files, loading the local tutor model, warming the tutor, and warming pronunciation. It also says `The first preparation may take a few minutes.` and shows a live elapsed timer. This moves deferred Metal/Core ML first-inference work ahead of the first card. After that gate disappears, individual pronunciations run without progress UI and only failures are shown. This is local initialization/device compilation, not a model download.
...
Word explanations are generated completely on-device by a bundled, text-only 4-bit build of [Qwen3.5-2B](https://huggingface.co/Qwen/Qwen3.5-2B). The app uses the [MLX Swift LM](https://github.com/ml-explore/mlx-swift-lm) runtime, a validated JSON response contract, and a narrow prompt to produce the target's most common established sense. Explanations begin directly with the meaning instead of a formula such as “the word means,” and include a part-of-speech label, one natural example sentence, up to three close synonyms when accurate ones exist, and an optional one- or two-sentence memory aid. Technical names normally omit synonyms rather than showing merely related terms. Generated content can still be imperfect. The result is cached in the existing Core Data store and may synchronize through Core Data/CloudKit; inference itself never uses a server.
